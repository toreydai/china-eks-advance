...
| Lab13 SA Token | 会在共享集群 `adv-shared` 上创建绑定 `cluster-admin` 的永不过期 Token，验证完必须立即清理，不要留在共享集群过夜 |
| Lab14 Custom Networking | 需要独立经典托管节点组集群 `eks-custom-net-demo`，不能在 Auto Mode 的 `adv-shared` 上跑 |
